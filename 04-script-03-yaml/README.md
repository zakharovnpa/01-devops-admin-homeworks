...
#  запись в отдельные файлы, с названием сервиса и актуальным IP адресом на момент открытия файла на просмотр и перезаписью файла при смене IP сервиса
   # json
        with open(fpath+host+".json",'w') as jsf:   # открываем на запись файл с именнем, состоящим из пути к файлу с названием проверяемого хоста и с расширением                                                     # ".json" и сохраним открытый файл под именем "jsf"
          json_data= json.dumps({host:ip})          # создаем переменную "json_data" и присваиваем ей параметры ключ:значения с помощью модуля Pyhon "json" и                                                         # метода "dumps"
                                                    # 
          jsf.write(json_data)                      # Записываем во временный файл "jsf" значения переменной "json_data". В ней сайчас записан хост и его IP адрес
